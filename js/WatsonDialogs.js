...
document.write('\
    ...
    *************************************************************************************************\
    -->\
	<div id="dialog-modal-alert" title="Alert box" style="display:none">\
	<label id="alertLabel"></label>\
	<input type="button" value="OK" text="this is text" class="OKBtnAlert WButton"/>\
	</div>\
');
...
$(document).on('click', "input.OKBtnAlert" , function() 
{
	WDLCloseFlag = true;
...
function openStringPad(title, instructions)
{
	...
	$("#dialog-modal-string").dialog("open");
	return dialogsDefer.promise();
}
...
function openSelector(title, array)
{
	...
	//Resets data string.
	$( "#VarsDialogSelect" ).data("selected", null);
	$("#VarsDialogSelect").find('option').remove();
	dialogmodalVars = $( "#dialog-modal-Vars" ).dialog("open");
...
function openAlert(title, string)
{
	WDLCloseFlag = false;
	dialogsDefer = $.Deferred();
	var Alert = $("#dialog-modal-alert");
...
function numExample()
{
	openNumPad(30, null, "This is a test", "Do things", false, 10).done(function(returned) {
		openNumPad(null, 30, "This is a test", "Do things", false, 10);
		alert(returned);
	});
}
...
function hexExample()
{
	openNumPad(-1, 1000, "Hexpad test", "Do more things", false, 16).done(function(returned) {
		openNumPad(-1, -1, "Hexpad test", "Do more things", false, 16).done(function(returned) {
			alert(returned);
		});
		alert(returned);
	});
...
function selectorExample()
{
	...
	openSelector("This is a selector", list).done(function(returned)
	{
		...
		list[0] = "choose again";
		list[1] = "yeah";
		openSelector("t", list).done(function(returned)
	{
		alert(returned);
	});
		alert(returned);
...
function stringpadExample()
{
	...
	{	
		openStringPad("This is a stringpad", "These are the instructions");
		alert(returned);
	});
}
...
{
	openAlert("Title", "text").done(function(returned)
	{
		openAlert("another title", "more text");
		alert(returned);
	});
}
